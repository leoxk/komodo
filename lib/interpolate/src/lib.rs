...
mod tests {
  ...
  fn stack_environment_collects_only_sensitive_values() {
    ...
    assert!(replacers.iter().any(|(_, key)| {
    ...
    assert!(sensitive_environment_key("STRIPE_API_SECRET"));
    assert!(sensitive_environment_key("POSTGRES_PASSWORD"));
  ...
  fn stack_interpolation_redacts_synthetic_marker() {
    ...
    let sanitized =
      svi::replace_in_string(&rendered, &interpolator.secret_replacers);
